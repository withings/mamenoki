...
pub async fn main() {
    ...
    tokio::join!(
        ...
        async {
            ...
            match writer_client
                .use_tube("example_connection_and_request")
                .await
            {
                Ok(_) => {
                    ...
                    loop {
                        ...
                        tokio::time::sleep(Duration::from_millis(PRODUCER_WAIT_TIME_MILLISEC))
                            .await;
                    }
                }
                Err(e) => {
                    log::error!(
                        "failed to use beanstalkd tube on webservice connection: {}",
                        e
                    );
                    std::process::exit(1);
                }
        ...
        async {
            ...
            match watch_client
                .watch_tube("example_connection_and_request")
                .await
            {
                Ok(_) => {
                ...
                Err(e) => {
                    log::error!(
                        "failed to watch beanstalkd tube on forwarder connection: {}",
                        e
                    );
                    std::process::exit(1);
                }
